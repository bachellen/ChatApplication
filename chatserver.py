...
class ChatServer:
    ...
    def send_message(self, target_client_id, source_socket, message):
        with self.lock:
            if target_client_id in self.clients:
                target_socket = self.clients[target_client_id]
                target_socket.sendall(f"[{source_socket.getpeername()}] {message}".encode())
            else:
                source_socket.sendall(b"Target client is not online\n")
